        ...
        >
          <div>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-white/80 px-4 py-2 text-xs font-bold uppercase tracking-[0.22em] text-blue-700 shadow-sm backdrop-blur">
              <Search size={14} />{t("Search & AI visibility")}{" "}</div>
            <h2 className="max-w-4xl text-5xl font-black tracking-[-0.055em] text-gray-950 sm:text-6xl lg:text-7xl">{t("Built to be found.")}{" "}<span className="block bg-gradient-to-r from-violet-400 via-fuchsia-300 to-cyan-300 bg-clip-text text-transparent">{t("Structured to be understood.")}{" "}</span>
            </h2>
          </div>
          <p className="max-w-xl text-lg leading-8 text-gray-600 lg:justify-self-end lg:pb-2 sm:text-xl">{t("Visibility is not a plugin added at launch. It starts in the architecture—with fast pages, meaningful structure and content that search engines can confidently interpret.")}{" "}</p>
        </motion.header>
              ...
              >
                <div className="mb-8 flex items-start justify-between">
                  ...
                  <span className="font-mono text-xs font-bold tracking-[0.2em] text-gray-400">
                    0{index + 1} / {t(style.label)}
                  </span>
                </div>
                <h3 className="mb-6 text-2xl font-bold tracking-tight text-gray-950">{t(feature.category)}</h3>
        ...
        >
          <div className="border-b border-white/10 p-7 sm:p-10 lg:border-b-0 lg:border-r">
            <div className="mb-5 flex items-center gap-3 text-sm font-bold uppercase tracking-[0.18em] text-fuchsia-200">
              <Sparkles size={18} />{t("Beyond traditional search")}{" "}</div>
            <h3 className="text-3xl font-black tracking-[-0.035em] sm:text-4xl">{t("Generative Engine Optimization")}</h3>
          </div>
          <div className="p-7 sm:p-10">
            <p className="text-lg leading-8 text-slate-200">{t("GEO makes expertise easier for AI assistants to identify, understand and cite. Clear entities, direct answers and machine-readable context help your content become a trusted source.")}{" "}</p>
            <a href="#contact" className="mt-7 inline-flex items-center gap-2 font-bold text-white hover:text-cyan-200">{t("Build a discoverable product")}{" "}<ArrowUpRight size={18} />
            </a>
          </div>
        </motion.div>
      </div>
